...
- Explicit subtitle generation for a selected local audio/video file up to 25 MB through configured OpenAI or Groq transcription, with provider-specific speech models, bounded OpenAI SSE partial text after submission, optional translation, editable fallback timing for untimed streams, a local overlap-aware cue timeline, whole-timeline shift/split/merge/delete and Undo/Redo controls, bilingual preview, cancellation, and SRT/VTT export.
