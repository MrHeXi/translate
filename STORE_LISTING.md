...
- Text-based document translator for pasted text, text files, Markdown, HTML files, JSON string values, DOCX paragraph text, EPUB spine documents, subtitle files, and simple PDFs with layout block metadata.
...
- The document translator handles text-based documents, HTML body text extraction, JSON string value extraction, DOCX paragraph text, EPUB spine documents, and simple text-based PDFs. Scanned PDFs, structure-preserving JSON rewrite, layout-preserving DOCX/EPUB rewrite, and full visual PDF rendering need later batches.
