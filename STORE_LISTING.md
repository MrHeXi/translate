...
- Manual image text translation using browser OCR or bundled offline OCR, plus SVG and accessibility text fallbacks, with click, freeform `Z` lasso, right-click, hover, visible-image batch, and explicit Scan then Translate chapter entry points; includes bounded source polygons, target-aware vertical CJK layout, per-image Retranslate/Apply/Undo, reconstructed-canvas PNG download, bounded local comic-bubble reconstruction when safe, and non-destructive OCR block overlays otherwise.
