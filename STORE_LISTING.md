...
- Text-based document translator for pasted text, text files, Markdown, HTML files, JSON string values with structure-preserving export, DOCX paragraph text, EPUB spine documents, subtitle files with timing-preserving export, and simple PDFs with layout block metadata.
...
- The document translator handles text-based documents, HTML body text extraction, JSON string value extraction and structure-preserving export, DOCX paragraph text, EPUB spine documents, subtitle file timing-preserving export, and simple text-based PDFs. Scanned PDFs, layout-preserving DOCX/EPUB rewrite, and full visual PDF rendering need later batches.
