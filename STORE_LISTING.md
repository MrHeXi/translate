...
- User-invoked Chrome side panel for text translation, available from the popup or `Alt+S`, with no translation request on panel open.
- Document translator for pasted text, text files, Markdown, HTML, JSON, DOCX, EPUB, subtitle files with timing-preserving export, and PDFs with local PDF.js rendering, positioned translations, browser-plus-bundled OCR fallback, and flattened translated-PDF export.
...
- Open the side panel from the popup header or with `Alt+S`; opening it never sends a translation request, while Translate or `Ctrl+Enter` submits only the entered text.
