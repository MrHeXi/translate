...
- Explicit subtitle generation for a selected local audio/video file up to 25 MB through separately configured OpenAI, Groq, or Deepgram transcription, with provider-specific speech models, bounded OpenAI SSE partial text after submission, provider-timed Deepgram Nova segments, optional translation, editable fallback timing for untimed streams, a local overlap-aware cue timeline, whole-timeline shift/split/merge/delete and Undo/Redo controls, bilingual preview, cancellation, and SRT/VTT export.
