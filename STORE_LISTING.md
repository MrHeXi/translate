...
- Translation, AI writing, and local-media transcription requests are sent only after the user explicitly submits the corresponding text, page, document, image, caption, or selected media file.
- The subtitle generator uploads only a user-selected file after Generate subtitles; media bytes stay in temporary memory and are cleared after completion, cancellation, disconnection, or error.
...
- `https://api.openai.com/*`: send user-requested text or an explicitly selected local media file to the configured OpenAI endpoint.
...
- Single purpose: user-triggered web and document text translation, selected-text translation, selected or currently visible image text translation, available subtitle/live-caption text translation, explicit local-media transcription and local transcript export, vocabulary collection, and vocabulary review.
...
- Personal or sensitive user data: no payment data, health data, or financial data is collected by the extension. A user-selected media file may contain personal content and is sent only after Generate subtitles is clicked.
- Website, document, and media content: only text needed for a requested translation and media explicitly submitted for transcription are sent to the provider selected by the user.
- User activity: learning progress and review stats are stored for product functionality, not default analytics.
- Data sharing: translation text and explicitly submitted local media are shared only with the selected translation or transcription provider after the corresponding user action.
