...
Translation provider API keys, client/application IDs, and temporary session tokens are stored separately in `chrome.storage.local`. They are not written to Chrome Sync, learning-data exports, translation cache keys, or extension logs. The settings page receives only masked credential summaries and never reads saved full credentials back from storage.
