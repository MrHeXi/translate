...
Translation provider requests happen only after the user asks LexiBridge to translate side-panel text, run an AI side-panel writing action, translate selected text, document text, selected image text, eligible currently visible image text, available video subtitle text, visible live caption text, or a page.
