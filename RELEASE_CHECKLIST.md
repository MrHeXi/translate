...
- [ ] Configure OpenAI or Groq, select a supported local media file under 25 MB, click Generate subtitles, edit cue text and start/end times, inspect overlap markers in the proportional cue timeline, shift the whole timeline, split at the original-text cursor, merge adjacent cues, delete a cue, Undo/Redo, and export the edited values as SRT/VTT. Confirm timeline clicks only focus an editor row, local edits open no connection and send no provider request, overflow merges lose no text, and Cancel stops the temporary upload or active per-cue translation request.
