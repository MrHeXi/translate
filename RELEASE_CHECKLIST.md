...
- [ ] Configure OpenAI, Groq, or Deepgram in the separate speech settings, select a supported local media file under 25 MB, click Generate subtitles, edit cue text and start/end times, inspect overlap markers in the proportional cue timeline, shift the whole timeline, split at the original-text cursor, merge adjacent cues, delete a cue, Undo/Redo, and export the edited values as SRT/VTT. Confirm selecting the file sends no media, Deepgram Nova keeps timed segments, local edits open no connection and send no provider request, overflow merges lose no text, and Cancel stops the temporary upload or active per-cue translation request.
