...
- [ ] Claim only verified, bounded document, explicit document-batch, video subtitle, explicit local-media/current-tab transcription, live caption, and image text features; describe flattened PDF output as the default and interaction-preserving output as an explicit option limited to supported AcroForm fields, widgets, links, and annotations, with signatures rejected and no silent fallback. Describe MOBI/AZW3 as bounded text import rather than eBook rewriting, bundled OCR as local but accuracy-dependent, and comic reconstruction as limited to safe regular bubbles after explicit action. Describe whole-chapter manga support as an explicit bounded Scan then Translate workflow for recognized/current-page reader structures. Do not claim editable PDF reflow, guaranteed scanned-PDF OCR, automatic/page-load manga translation, universal site coverage, unbounded chapters, guaranteed image inpainting, background or automatic tab-audio capture, meeting bots, or account cloud sync.
...
- [ ] Repeat masking with valid PKCS#8, encrypted PKCS#8, RSA, DSA, EC, and OpenSSH PEM private keys, AWS `AKIA`/`ASIA` IDs, classic and stateless `ghs_` GitHub tokens, and scoped OpenAI keys. Confirm complete-key masking, exact restoration, malformed/example rejection, and the shared match limit.
...
- [ ] Confirm PDF selection rejects sources above 64 MiB, more than 1,000 pages, a page above 32 Mi rendered pixels, and documents above 1,024 Mi rendered pixels before unsafe canvas allocation.
- [ ] Load a PDF with an editable text field, widget, URI link, and text annotation; enable Preserve forms, links, and annotations, click Export PDF, and confirm those interactions remain usable. Repeat with a signed PDF, outline JavaScript, automatic actions, page-associated or embedded files, a non-HTTP(S) link, a dangerous annotation, an over-limit object graph, and a forced preservation mismatch; confirm each unsafe case fails visibly without downloading a flattened substitute.
...
- [ ] Select an image-only or mixed PDF; confirm selection and scan-setting changes run no OCR, then click Translate document and verify grayscale/binary cleanup plus at most one routed second local OCR language.
...
- [ ] Load a PDF and click Export BabelDOC guide. Confirm the button is hidden for non-PDF/history-only content, selection sends no request, the guide contains sanitized placeholder paths and quoted PowerShell/POSIX commands, and no real path, API key, upload, or command execution occurs.
...
- [ ] Configure OpenAI, Groq, Deepgram, or Cloudflare Workers AI in the separate speech settings, select a supported local media file under 25 MB, click Generate subtitles, edit cue text and start/end times, inspect overlap markers in the proportional cue timeline, shift the whole timeline, split at the original-text cursor, merge adjacent cues, delete a cue, Undo/Redo, and export the edited values as SRT/VTT. Confirm selecting the file sends no media, Deepgram Nova and Cloudflare Whisper keep provider timing when supplied, local edits open no connection and send no provider request, overflow merges lose no text, and Cancel stops the temporary upload or active per-cue translation request.
- [ ] Save a 32-hex Cloudflare Account ID and API Token in the local speech settings; confirm only masked summaries are shown, the endpoint remains fixed at `https://api.cloudflare.com/client/v4/accounts`, opening settings or the subtitle generator sends no request, automatic language with empty context sends raw media, explicit language/context sends documented structured JSON, encoding yields to Cancel before fetch, success JSON is capped at 4 MiB, error JSON at 64 KiB, token redaction occurs before truncation, and a corrupted dedicated configuration can still be removed.
- [ ] Return oversized JSON from OpenAI/Groq, Deepgram, and Cloudflare transcription mocks; confirm each response is rejected by its byte limit before native JSON parsing.
- [ ] On Twitch, Dailymotion, TED, and Prime Video, confirm the dedicated adapter resolves only exact domains/subdomains, rejects lookalikes, prefers site selectors, and never starts translation before Start.
- [ ] On Slack Huddles, Jitsi Meet, and BigBlueButton, confirm visible captions translate only after Start, speaker labels remain local, and Stop aborts any pending caption request immediately.
- [ ] Select local media and confirm no AudioContext is created until Generate waveform; verify the button becomes Stop waveform, Stop/file replacement/page close ignores late decode results, and 23.976/24/25/29.97/30/50/59.94/60 fps snapping sends no provider request.
- [ ] Switch between provider-specific speech models. Confirm Whisper models keep provider segments, GPT-4o Transcribe models display bounded SSE partial text only after Generate/Stop and generate, untimed completion says editable fallback timing, and Cancel hides the preview and ignores late events.
- [ ] Load safe FLAC and OGG samples and confirm selection stays idle until Generate subtitles.
