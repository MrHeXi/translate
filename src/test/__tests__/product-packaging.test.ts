...
describe('product packaging contract', () => {
  ...
  it('ships release-ready user documentation and privacy disclosure', () => {
    ...
    expect(readme).toContain(
      'Google Meet, Zoom, Microsoft Teams, Webex, Slack Huddles, Jitsi Meet, and BigBlueButton-style caption containers'
    );
    expect(readme).toContain('Twitch, Dailymotion, TED, and Prime Video');
    ...
    expect(readme).toContain('export a bounded BabelDOC local-workflow guide');
    expect(readme).toContain('OpenAI GPT-4o Transcribe models can display bounded SSE partial text');
    expect(readme).toContain('stateless `ghs_` format');
  ...
  it('tracks the expanded Immersive Translate replication scope', () => {
    ...
    expect(roadmap).toContain('mark Tencent Cloud TMT text translation unavailable');
    expect(roadmap).toContain('mark Reverso unavailable');
    ...
    expect(roadmap).toContain(
      'Done: add contract-tested dedicated adapters for Netflix, Vimeo, Bilibili, Udemy, Coursera, Khan Academy, Nebula, Bloomberg, Twitch, Dailymotion, TED, and Prime Video'
    );
    expect(roadmap).toContain('Deepgram as an independent timed speech provider');
    expect(roadmap).toContain('Slack Huddles, Jitsi Meet, and BigBlueButton live-caption adapters');
    ...
    expect(roadmap).toContain('Done: add an explicit bounded BabelDOC-compatible local workflow guide');
    expect(roadmap).toContain('OpenAI SSE partial text after explicit submission');
    ...
    expect(roadmap).toContain('stateless `ghs_` JWT format');
  ...
  it('packages the local PDF runtime and document-page controls', () => {
    ...
    expect(documentHtml).toContain('id="exportBabelDocGuide"');
    ...
    const babelDocWorkflow = readProjectFile('src/services/BabelDocWorkflow.ts');
    expect(babelDocWorkflow).toContain('BABELDOC_GUIDE_MAX_CODE_POINTS = 8_192');
    expect(babelDocWorkflow).toContain('C:\\\\ABSOLUTE\\\\PATH');
    expect(babelDocWorkflow).toContain('This is a local command guide, not an API integration');
    ...
    expect(optionsHtml).toContain('id="aiToolsCatalogItem"');
    expect(optionsHtml).toContain('id="installAiToolsCatalogItem"');
    const aiToolsCatalog = readProjectFile('src/services/AiToolsCatalogService.ts');
    expect(aiToolsCatalog).toContain('AI_TOOLS_CATALOG_VERSION');
    expect(aiToolsCatalog).toContain('prepareSelection');
    expect(aiToolsCatalog).not.toContain('fetch(');
    ...
    expect(sidePanelHtml).toContain('data-mode="proofread"');
    expect(sidePanelHtml).toContain('data-mode="explain"');
    expect(sidePanelHtml).toContain('data-mode="extract"');
    expect(sidePanelHtml).toContain('data-mode="academic"');
    ...
    expect(subtitlesHtml).toContain('id="transcriptionModel"');
    expect(subtitlesHtml).toContain('id="partialTranscript"');
    ...
    expect(subtitlesHtml).toContain('id="timelineShift"');
    expect(subtitlesHtml).toContain('id="applyTimelineShift"');
    expect(subtitlesHtml).toContain('id="undoTimelineEdit"');
    expect(subtitlesHtml).toContain('id="redoTimelineEdit"');
    expect(subtitlesHtml).toContain('id="timelineOverview"');
    expect(subtitlesHtml).toContain('id="timelineTrack"');
    expect(subtitlesHtml).toContain('id="applyToSourceVideo"');
    expect(subtitlesHtml).toContain('id="clearSourceVideo"');
    expect(subtitlesScript).toContain("action: 'getVideoPlaybackPosition'");
    expect(subtitlesScript).toContain("action: 'applyGeneratedVideoSubtitles'");
    expect(subtitlesScript).toContain("action: 'clearGeneratedVideoSubtitles'");
    expect(subtitlesScript).toContain('sourceNavigationToken');
    ...
    expect(subtitlesScript).toContain('shiftGeneratedSubtitleCues');
    ...
    expect(contentScript).toContain("'applyGeneratedVideoSubtitles'");
    expect(contentScript).toContain("'clearGeneratedVideoSubtitles'");
    expect(videoSubtitleTranslator).toContain('expectedNavigationToken?: unknown');
    expect(videoSubtitleTranslator).toContain('clearGeneratedVideoSubtitles(');
    expect(videoSubtitleTranslator).not.toContain('.play()');
    ...
    expect(sensitiveDataMasker).toContain('PRIVATE_KEY_PEM_PATTERN');
    expect(sensitiveDataMasker).toContain('AWS_ACCESS_KEY_ID_PATTERN');
    expect(sensitiveDataMasker).toContain('GITHUB_TOKEN_PATTERN');
    expect(sensitiveDataMasker).toContain('OPENAI_SCOPED_KEY_PATTERN');
    expect(sensitiveDataMasker).toContain('GITLAB_TOKEN_PATTERN');
    expect(sensitiveDataMasker).toContain('SLACK_TOKEN_PATTERN');
    expect(sensitiveDataMasker).toContain('STRIPE_SECRET_KEY_PATTERN');
    expect(sensitiveDataMasker).toContain('GOOGLE_API_KEY_PATTERN');
    expect(sensitiveDataMasker).toContain('AZURE_STORAGE_CONNECTION_PATTERN');
    expect(sensitiveDataMasker).toContain('DATABASE_URI_PASSWORD_PATTERN');
    expect(sensitiveDataMasker).toContain('CHINESE_RESIDENT_ID_PATTERN');
    const platformCapabilities = readProjectFile('src/services/PlatformCapabilityService.ts');
    expect(platformCapabilities).toContain("'safari'");
    expect(platformCapabilities).toContain("'userscript'");
    expect(platformCapabilities).toContain("'zotero'");
    expect(platformCapabilities).toContain("'ios'");
    expect(platformCapabilities).toContain("'android'");
    expect(platformCapabilities).toContain('cannot start automatically');
    ...
    expect(generatedSubtitleDocument).toContain('GENERATED_SUBTITLE_MAX_TEXT_CODE_POINTS');
    expect(generatedSubtitleDocument).toContain('shiftGeneratedSubtitleCues');
    expect(generatedSubtitleDocument).toContain('splitGeneratedSubtitleCue');
    expect(generatedSubtitleDocument).toContain('mergeGeneratedSubtitleCues');
    expect(generatedSubtitleDocument).toContain('createGeneratedSubtitleTimelineLayout');
